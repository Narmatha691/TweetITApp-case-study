...
namespace MyTwitterAPI.Services
{
    public class PostService : IPostService
    {
        ...
        public PostDTO GetPostById(int postId)
        {
            try
            {
                Post postWithUser = context.Posts.Where(p => p.PostId == postId && p.User.Type != "Blocked")
        ...
        public List<PostDTO> SearchPostsByTitleAndUserId(string userId, string searchTerm)
        {
            try
            {
                ...
                List<Post> activeMatchingPosts = context.Posts
                     .Include(p => p.User)
                     .Where(p => p.UserId == userId && p.User.Type != "Blocked" &&
        ...
        public List<PostDTO> GetPostsByUserId(string userId)
        {
            try
            {
                List<Post> activeUserPosts = context.Posts
                    .Include(p => p.User)
                    .Where(p => p.UserId == userId && p.Active == 1 && p.User.Type != "Blocked")
                    .OrderByDescending(p => p.DateandTime)
                    .ToList();
        ...
        public List<PostDTO> GetPostsOfFollowing(string userId)
        {
            try
            {
                List<Post> followingPosts = context.Followers
                    ...
                    .OrderByDescending(p => p.DateandTime)
                    .ToList();
                List<PostDTO> userPostDTOs = _mapper.Map<List<PostDTO>>(followingPosts);
        ...
        public List<PostDTO> SearchPostsOfFollowing(string userId,string searchterm)
        {
            try
            {
                List<Post> followingPosts = context.Followers
                    ...
                    .Include(p => p.User)
                    .Where(p => p.Active == 1 && p.User.Type != "Blocked" && EF.Functions.Like(p.Title, $"%{searchterm}%"))
                    .OrderByDescending(p => p.DateandTime)
                    .ToList();
                List<PostDTO> userPostDTOs = _mapper.Map<List<PostDTO>>(followingPosts);
